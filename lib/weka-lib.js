...
//          console.log(stdout);
  ...
  /**
   * Makes a prediction on the given model and test data
   *
   * @param model string with the path to the model file
   * @param test string with the path to the csv/arff file with the test data
   * @param options Parameter for the classifier
   * @param cb Callback function
   */
      ...
      function (callback) {
        ...
        child = exec('java -classpath ./bin/weka.jar ' + options.classifier + ' '
          ...
          function (error, stdout, stderr) {
            ...
            result.actual     = splitted[1];
            result.predicted  = splitted[2];
            result.error      = splitted[3];
